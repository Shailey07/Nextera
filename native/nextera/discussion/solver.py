...
from .. import config
from ..llm.connector import GroqConnector, GeminiConnector, PerplexityConnector
...
class DiscussionPromptSolver(object):
    ...
    def solve(self) -> bool:
        ...
        answer = connector.get_response(
            self.format_llm_prompt(prompt), system_prompt=SYSTEM_PROMPT)
    ...
    def get_prompt(self) -> dict | None:
        ...
        if not course_forum_question_id:
            logger.warning("Could not determine courseForumQuestionId.")
            return None
    ...
    def submit_answer(self, course_forum_question_id: str, value: str, max_retries: int = 5) -> bool:
        if max_retries <= 0:
        ...
        res = self.session.post(
            ...
            json={
                "content": {
                    "typeName": "cml",
                    "definition": {"dtdId": "discussion/1", "value": value}},
                "courseForumQuestionId": course_forum_question_id})
        ...
        if "FORUM_POST_RATE_LIMIT_REACHED_EXCEPTION" in res.text:
            logger.warning("Forum rate limit — waiting 60s...")
            random_delay(60.0, 90.0)  # 60-90 second wait
